...
class AppStore extends Component {
  ...
  getTabApps = serachedList => {
    const {activeTabId} = this.state
    const tabApps = serachedList.filter(
  ...
  render() {
    ...
    const searchResults = this.searchApps()
    const filteredApps = this.getTabApps(searchResults)
    return (
      <div className="app-container">
        <div className="content-container">
          <h1 className="store-heading">App Store</h1>
          ...
          <ul className="apps-list">
            {filteredApps.map(eachApp => (
              <AppItem key={eachApp.appId} appDetail={eachApp} />
            ))}
          </ul>
        </div>
      </div>
